...
**Parsing rules**: Parse `--max-rounds`, `--max-phases`, and `--resume` from `$ARGUMENTS`. Non-numeric, empty, or missing values → fall back to defaults. Clamp to valid ranges. Example: `--max-rounds=0` → 1; `--max-rounds=abc` → 10; `--max-phases=100` → 20.
