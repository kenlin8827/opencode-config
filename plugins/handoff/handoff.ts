/**
 * Handoff Plugin — registers the `/handoff` slash command programmatically
 * via the `config` hook (same pattern as goal.ts, grill-me.ts and
 * review-fix-loop.ts — no `commands/handoff.md` file needed), then injects
 * the handoff protocol into the system prompt.
 *
 * Two hooks:
 ...
 *
 * The protocol body lives in `handoff.md` (next to this file).
 *
 * This module is re-exported by `plugins/handoff.ts` (barrel) so
...
const PROTOCOL_FILE = join(__dirname, "handoff.md")
const COMMAND_NAME = "handoff"
const MARKER = "[HANDOFF SKILL]"
...
export const HandoffPlugin: Plugin = async ({ client }) => ({
  config: async (cfg) => {
    ...
    cfg.command[COMMAND_NAME] = {
      template: "/handoff $ARGUMENTS",
      description:
        "Handoff — compact the current conversation into a handoff document so a fresh session can pick up the work. Usage: /handoff [focus for the next session]",
      agent: "build",
    }
  },
  ...
  "experimental.chat.system.transform": async (
    ...
    if (!await scoped(input, output.system, "handoff", client)) return
