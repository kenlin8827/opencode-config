function lookup(store, id) {
    return typeof store === 'function' ? store(id) : store[id];
}
function hasOriginal(msg) {
    const orig = msg.metadata?._cce_original;
    return Array.isArray(orig?.ids) && orig.ids.length > 0;
}
function expandOnce(messages, store) {
    const out = [];
    let expanded = 0;
    let passthrough = 0;
    const missingIds = [];
    for (const msg of messages) {
        ...
        const ids = msg.metadata._cce_original.ids;
        const originals = [];
        const missing = [];
        for (const id of ids) {
 ...
 */
export function uncompress(messages, store, options) {
