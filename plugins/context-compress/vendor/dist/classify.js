...
const YAML_RE = /^[\w-]+:\s+.+\n[\w-]+:\s+.+/m;
const POETRY_RE = /\n[A-Z][^.!?\n]*\n[A-Z][^.!?\n]*\n[A-Z][^.!?\n]*(?:\n|$)/;
function detectStructuralPatterns(text) {
    const reasons = [];
    if (CODE_FENCE_RE.test(text))
        reasons.push('code_fence');
    if (INDENT_CODE_RE.test(text))
        reasons.push('indented_code');
    if (LATEX_RE.test(text))
        reasons.push('latex_math');
    if (UNICODE_MATH_RE.test(text))
        reasons.push('unicode_math');
    if (JSON_RE.test(text))
        reasons.push('json_structure');
    if (YAML_RE.test(text))
        reasons.push('yaml_structure');
    if (POETRY_RE.test(text))
        reasons.push('verse_pattern');
    ...
    if (cv > 1.2 && lines.length > 3)
        reasons.push('high_line_length_variance');
...
function detectSqlContent(text) {
    ...
    for (const kw of distinct) {
        if (SQL_STRONG_ANCHORS.has(kw))
            return true;
    }
    // 3+ distinct keywords AND 1+ weak anchor → likely SQL
    if (distinct.size >= 3) {
        for (const kw of distinct) {
            if (SQL_WEAK_ANCHORS.has(kw))
                return true;
        }
    }
    return false;
...
function detectContentTypes(text) {
    const reasons = [];
    // SQL keyword density
    if (detectSqlContent(text))
        reasons.push('sql_content');
