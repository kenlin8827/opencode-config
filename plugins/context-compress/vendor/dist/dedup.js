 ...
 */
export function analyzeDuplicates(messages, recencyStart, preserveRoles) {
    ...
    for (const indices of hashGroups.values()) {
        ...
        for (const group of exactGroups.values()) {
            ...
            const contentLength = messages[keepIdx].content.length;
            for (const idx of group) {
                if (idx !== keepIdx) {
 ...
 */
export function analyzeFuzzyDuplicates(messages, recencyStart, preserveRoles, exactAnnotations, threshold) {
    const annotations = new Map();
    const eligible = [];
    for (let i = 0; i < messages.length; i++) {
        const msg = messages[i];
    ...
    for (const group of groups.values()) {
        ...
        for (const ei of group) {
            ...
            const sim = simLookup.get(`${ei}:${keepEi}`) ??
...
function jaccardLines(a, b) {
    const freqA = new Map();
    for (const line of a)
        freqA.set(line, (freqA.get(line) ?? 0) + 1);
    const freqB = new Map();
    for (const line of b)
        freqB.set(line, (freqB.get(line) ?? 0) + 1);
    let intersection = 0;
    let union = 0;
