...
import { analyzeDuplicates, analyzeFuzzyDuplicates } from './dedup.js';
/**
 ...
 */
function collectParentIds(msgs) {
    const parents = [];
    for (const m of msgs) {
...
function scoreSentence(sentence) {
    ...
    // Emphasis phrases
    if (EMPHASIS_RE.test(sentence))
        score += 4;
    // Numbers with units
    score +=
        (sentence.match(/\b\d+(?:\.\d+)?\s*(?:seconds?|ms|MB|GB|TB|KB|retries?|workers?|threads?|nodes?|replicas?|requests?|%)\b/gi) ?? []).length * 2;
    ...
    if (sentence.length >= 40 && sentence.length <= 120)
        score += 2;
    // Filler penalty
    if (FILLER_RE.test(sentence.trim()))
        score -= 10;
    return score;
}
function summarize(text, maxBudget) {
...
function summarizeStructured(text, maxBudget) {
    const lines = text.split('\n');
    const nonEmpty = lines.filter((l) => l.trim().length > 0);
...
export function defaultTokenCounter(msg) {
    return Math.ceil(contentLength(msg) / 3.5);
}
/** Build a compressed message with _cce_original provenance metadata. */
function buildCompressedMessage(base, ids, summaryContent, sourceVersion, verbatim, sourceMessages) {
...
function formatSummary(summaryText, rawText, mergeCount, skipEntities, summaryId) {
...
function collectGroup(classified, startIdx) {
    const group = [];
...
function computeStats(originalMessages, resultMessages, messagesCompressed, messagesPreserved, sourceVersion, counter, messagesDeduped, messagesFuzzyDeduped) {
...
// ---------------------------------------------------------------------------
function compressSync(messages, options = {}) {
    ...
    let dedupAnnotations = (options.dedup ?? true) ? analyzeDuplicates(messages, recencyStart, preserveRoles) : undefined;
    if (options.fuzzyDedup) {
        const fuzzyAnnotations = analyzeFuzzyDuplicates(messages, recencyStart, preserveRoles, dedupAnnotations ?? new Map(), options.fuzzyThreshold ?? 0.85);
...
// ---------------------------------------------------------------------------
async function withFallback(text, userSummarizer, maxBudget) {
    if (userSummarizer) {
        try {
...
async function compressAsync(messages, options = {}) {
    const sourceVersion = options.sourceVersion ?? 0;
    const counter = options.tokenCounter ?? defaultTokenCounter;
...
// ---------------------------------------------------------------------------
function sumTokens(messages, counter) {
    return messages.reduce((sum, m) => sum + counter(m), 0);
}
function budgetFastPath(messages, tokenBudget, sourceVersion, counter) {
...
function addBudgetFields(cr, tokenBudget, recencyWindow, counter) {
 ...
 */
function forceConvergePass(cr, tokenBudget, preserveRoles, sourceVersion, counter) {
    if (cr.fits)
        return cr;
    ...
    for (let i = 0; i < cutoff; i++) {
        ...
        if (m.role && preserveRoles.has(m.role))
            continue;
...
function compressSyncWithBudget(messages, tokenBudget, options) {
...
async function compressAsyncWithBudget(messages, tokenBudget, options) {
    const minRw = options.minRecencyWindow ?? 0;
    const sourceVersion = options.sourceVersion ?? 0;
    const counter = options.tokenCounter ?? defaultTokenCounter;
    const fast = budgetFastPath(messages, tokenBudget, sourceVersion, counter);
