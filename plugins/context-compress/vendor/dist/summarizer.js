...
function buildPrompt(text, maxResponseTokens, opts) {
...
export function createSummarizer(callLlm, options) {
    ...
    return (text) => callLlm(buildPrompt(text, maxResponseTokens, opts));
}
export function createEscalatingSummarizer(callLlm, options) {
    const maxResponseTokens = options?.maxResponseTokens ?? DEFAULT_MAX_RESPONSE_TOKENS;
    const baseOpts = {
        systemPrompt: options?.systemPrompt || undefined,
        preserveTerms: options?.preserveTerms,
