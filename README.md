...
User
 ...
 │
  ...
  │
  ├── @code (primary) ── direct developer, delegation only on request (advisor/explorer/code-review/vision)
  │
...
| `handoff.ts` (+ `plugins/handoff/` helpers) | `config` + `system.transform` | Registers `/handoff` slash command programmatically; injects handoff protocol (compact the conversation into an OS-temp-dir document, reference artifacts by path, redact secrets, suggest agents for the next session) from markdown into system prompt (LLM-only, not visible in chat UI). |
...
├── grill-me.ts               # Barrel entry (re-exports /grill-me plugin)
├── grill-with-docs.ts        # Barrel entry (re-exports /grill-with-docs plugin)
├── grill/
│   ├── grill-me.ts           # Plugin entry: /grill-me command + system.transform
