...
| **`/quick-dev <task>`** | 开发流 | **Quick-Dev 极速免审直通**：Flash 极速出码 + 动态领域灵魂注入（零审查开销，出码即交付，别名 `/flash-dev`，详见 [五档开发流](dev-loops.md)） |
| **`/fast-dev <task> [--max-rounds=N]`** | 开发流 | **Fast-Dev 敏捷单审闭环**：Flash 极速出码（动态领域灵魂） + 旗舰单审证据驱动严把关（详见 [五档开发流](dev-loops.md)） |
| **`/deep-dev <task> [--max-rounds=N]`** | 开发流 | **Deep-Dev 深度双审共识闭环**：Flash 编码 + 双旗舰顶级会审 + Advisor 争议仲裁共识，支持全栈拆解汇总（详见 [五档开发流](dev-loops.md)） |
| **`/ultra-dev <objective> [--max-rounds=N] [--max-phases=N]`** | 开发流 | **Ultra-Dev 自主多阶段闭环**：端到端自主执行 —— 将大型目标分解为多阶段，每阶段独立 `/deep-dev` 循环 + 上下文压缩 + 逐阶段 Git 提交隔离 + 支持 `--resume` 断点续跑（详见 [五档开发流](dev-loops.md)） |
