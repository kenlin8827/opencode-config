# 三阶闭环开发 (`/quick-dev` & `/fast-dev` & `/deep-dev`)

三阶闭环开发是 OpenCode 生产级工程化配置中的**核心王牌工作流体系**。

它开创性地采用了 **“Flash 模型极速写码 ➕ Flagship 旗舰模型 PUA 级严苛审查 ➕ 动态领域灵魂注入 ➕ 争议仲裁共识”** 的多 Agent 协同架构，构建了从**零审查直通**到**单审敏捷**再到**双审深度**的三级开发梯队，旨在实现**“开发速度提升 3 倍、Token 成本降低 80%、最终交付代码质量零妥协”**的工程奇迹。
...
三阶体系将**“出码执行（体力活）”**与**“需求质量把关（脑力活）”**彻底解耦，按需分层：
...
【极速免审直通 (出码即交付)】        【标准敏捷闭环 (日常开发主力)】     【深度双审共识闭环 (核心高危重器)】
        │                                  │                                  │
┌───────┴───────┐                  ┌───────┴───────┐                  ┌───────┴───────┐
│• 调度: 原样直通│                  │• 调度: 原样直通│                  │• 调度: 原样直通│
│• 编码: Flash  │                  │• 编码: Flash  │                  │• 编码: Flash  │
│• 审查: 无 (跳过)│                 │• 审查: 旗舰单审│                  │• 审查 A: 需求对齐│
│• 交付: 即刻交付│                  │• 轮次: 最多10轮│                  │• 审查 B: 安全防线│
│               │                  │• 交付: 单审通过│                  │• 仲裁: Advisor│
│               │                  │               │                  │• 交付: 双审共识│
└───────────────┘                  └───────────────┘                  └───────────────┘
```
...
| **收敛轮次** | **1 轮（直通）** | 默认上限 10 轮（通常 2~3 轮收敛） | 默认上限 10 轮（通常 3~5 轮收敛） |
...
## 四、 跨端/全栈复合任务流转 (Full-Stack Orchestration)
...
1. **Safety-First 原则**：在 `/deep-dev` 中，当 Reviewer A 与 Reviewer B 发生分歧且 `@advisor` 仲裁时，永远遵循**从严不从宽（Safety-First）**原则，宁可多改，不可漏放；
2. **10 轮防死锁熔断**：如果经过 10 轮迭代仍有未解决的争议点，系统会自动熔断并输出 **《未收敛问题报告》**，列出具体的争议焦点交由人工决策，绝不产生无限死循环；
3. **零配置侵入**：`tiers.json` 保持干净映射，平滑联动 `/profile` 切换任何模型服务商。
