...
三条线性开发流（`/quick-dev`、`/plan-dev`、`/review-dev`）已实现为同一个 [`/dev`](dev.md) 组合引擎的**预设**——每条都展开为 `/dev` 上的一组标志。`/prud-dev` 与 `/ultra-dev` 仍是独立协议（拓扑不同）。无预设匹配时可直接调用 `/dev`。
...
| 开发流 | 理念 | 适用场景 |
|---|---|---|
| `/quick-dev` ⚡ | **零摩擦** —— flash 档出码，审查可选 | 临时脚本、极简改动、样式微调、快速原型 |
| `/plan-dev` 📋 | **计划先行** —— 澄清需求、出计划、再实现（审查可选） | 日常默认：希望在编码前审批计划的功能 |
| `/review-dev` 🧠 | **深度共识** —— 双旗舰顶级会审 + 争议仲裁 | 20% 核心高危：分布式事务、全栈跨端、安全敏感 |
| `/prud-dev` 🛡️ | **风险优先** —— 编码前先做 FMEA，风险登记册驱动全流程 | 安全攸关场景：金融支付、核心鉴权、医疗航空，任何出错可能造成伤害的场景 |
| `/ultra-dev` 🛸 | **完全自主** —— 输入目标，多阶段自主执行，零交互 | 横跨多领域的大型系统交付 |
...
- **断点续跑**: `--resume` 读取 `.opencode/ultra-dev-state.md`
...
```
                 ...
                 是                            否
                 /                              \
           🛸 /ultra-dev                Bug 是否会伤害人身或业务？
           (完全自主)                    /                        \
                                       是                          否
                                       /                            \
                                🛡️ /prud-dev                  是否是核心高危场景
                                (风险优先)                   (分布式事务、全栈跨端)？
                                                           /                      \
                                                         是                                                        否
                                                         /                                                            \
                                                   🧠 /review-dev                                           编码前是否需要审批计划？
                                                   (深度共识)                                                  /              \
                                                                                                              是                否
                                                                                                              /                    \
                                                                                                        📋 /plan-dev          ⚡ /quick-dev
...
| 维度 | ⚡ `/quick-dev` | 📋 `/plan-dev` | 🧠 `/review-dev` | 🛡️ `/prud-dev` | 🛸 `/ultra-dev` | 🧩 `/dev` |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
所有涉及编码的开发流（`/quick-dev`、`/plan-dev`、`/review-dev`、`/prud-dev`、`/ultra-dev` 及 `/dev` 组合）均使用**动态领域灵魂注入**：

1. **无状态容器**：`@fast-coder` 静态绑定 Flash/Lite 档模型，保持高吞吐与极速响应（`/quick-dev` 预设与零深度标志的裸 `/dev` 使用）；
...
### `/quick-dev`（极速免审直通，可选审查）

```bash
/quick-dev 给代码块右上角添加一键复制按钮与 Toast 提示
/flash-dev 修复分页查询页码从 0 开始计算的越界问题  # 别名
/quick-dev 添加暗色模式切换 --review  # 带单审
```

### `/plan-dev`（计划先行，可选审查）

```bash
/plan-dev 实现用户个人中心头像上传与裁剪组件
/plan-dev 优化订单查询分页 SQL --review  # 计划 + 单审
/plan-dev 添加支付回调处理 --review --max-rounds=5
```

### `/prud-dev`（FMEA 审慎开发）

```bash
/prud-dev 实现带幂等保证的资金清结算 --top=5
/prud-dev 为移动端添加 OAuth2 PKCE 流程 --top=3 --max-rounds=8
```

### `/review-dev`（核心高危与全栈开发）

```bash
/review-dev 重构资金清结算分布式事务与幂等状态机
/review-dev 实现扫码登录全流程：会话表、轮询接口、弹窗组件 --max-rounds=10
```

### `/ultra-dev`（自主多阶段执行）

```bash
/ultra-dev 实现完整的用户认证系统：OAuth2、会话管理、RBAC
/ultra-dev 构建实时通知服务：WebSocket 网关、消息队列、客户端 SDK、管理后台 --max-rounds=8 --max-phases=10
/ultra-dev --resume  # 断点续跑
```
...
| 防线 | 适用开发流 |
|---|---|
| **Safety-First 仲裁** | `/review-dev`、`/ultra-dev`、`/prud-dev`（启用双审时） |
| **计划确认门禁** | `/plan-dev` |
| **10 轮防死锁熔断** | 所有含审查的开发流（`/ultra-dev` 为每阶段 10 轮） |
| **连续熔断停机**（≥ 3） | `/ultra-dev` |
| **阶段数上限护栏**（默认 6，上限 20） | `/ultra-dev` |
| **上下文压缩** | `/ultra-dev`（每 2 阶段） |
| **每阶段 Diff 隔离** | `/ultra-dev`（每阶段独立 git commit） |
| **风险登记册审计** | `/prud-dev`（Top-N 缓解措施全部验证） |
