...
| **`/git-merge <source> <target> [--dry-run] [--no-verify] [--squash] [--no-ff] [--abort]`** | Git Workflow | **Stock `git merge`, agent as the resolver**: syncs target to origin first (`git pull --ff-only`; divergence halts), creates guard backup branches, then merges source into target — git auto-merges everything it can; only conflicted files get hands-on semantic resolution against the true merge-base, where **target HEAD is the authoritative baseline** (target's changes — deletions included — stand; source layers on top and never silently undoes the baseline). Picking a strategy: land as one clean commit → `--squash`; otherwise → no flag (merge commit preserves both sides' topology). For a linear per-commit replay instead, use `/git-rebase`. Agent verifies once (per-hunk rationale + a whole-file coherence re-read + a per-hunk confidence self-check that escalates uncertain hunks to `@advisor` (best-effort) and hands anything it can't resolve confidently — ambiguous or advisor unavailable — to you, never guessing), reports a per-hunk decision log with confidence, and archives it to `.git/ocp-merge-reports/` when conflicts were resolved |
| **`/git-rebase <source> <target> [--dry-run] [--no-verify] [--continue] [--skip] [--abort]`** | Git Workflow | **Stock `git rebase`, agent as the resolver**: replays ALL of source's unique commits onto target's HEAD for a clean linear history — syncs target to origin (`git pull --ff-only`), guard-backs up **both** tips (rebase rewrites source), then `git rebase <target>` with each per-commit conflict resolved semantically (onto-branch HEAD is the authoritative baseline), then fast-forwards target. Rewrites history: landing target is a plain `git push`; if source was already pushed, updating it needs `git push --force-with-lease` (user's call). Agent verifies once (per-hunk rationale + a whole-file coherence re-read + a per-hunk confidence self-check that escalates uncertain hunks to `@advisor` (best-effort) and hands anything it can't resolve confidently — ambiguous or advisor unavailable — to you, never guessing), reports a per-commit decision log with confidence, and archives it to `.git/ocp-rebase-reports/` when conflicts were resolved |
| **`/git-pull [--rebase] [--dry-run] [--no-verify] [--abort]`** | Git Workflow | **Safe upstream sync for the current branch**: tries `git pull --ff-only` first (rewrites nothing); if diverged, creates a guard backup branch, then reconciles via the git-merge protocol with the fetched remote ref as source — the remote's published history is the authoritative baseline. `--rebase` replays local commits onto the remote tip instead. No `--squash` — squashing your own branch's published history is never legitimate |
