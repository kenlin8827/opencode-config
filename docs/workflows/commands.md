...
| **`/git-merge <source> <target> [--dry-run] [--no-verify] [--squash] [--rebase] [--no-ff] [--abort]`** | Git Workflow | **Stock `git merge`, agent as the resolver**: syncs target to origin first (`git pull --ff-only`; divergence halts), creates guard backup branches, then merges source into target — git auto-merges everything it can; only conflicted files get hands-on semantic resolution against the true merge-base, where **target HEAD is the authoritative baseline** (target's changes — deletions included — stand; source layers on top and never silently undoes the baseline). Three strategies: default merge (1 merge commit), `--squash` (all changes as 1 commit with LLM-generated message), `--rebase` (replay source commits individually, linear history). Agent verifies once, reports a per-hunk decision log |
