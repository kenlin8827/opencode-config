...
终端界面提供对话式交互，支持 21 位专家智能体（`@java-dev`、`@security`、`@dba`、`@frontend-dev`、`@fast-coder` 等）、三种工作模式（`@code` / `@build` / `@plan`）、工作流斜杠命令（`/quick-dev` · `/deep-dev` · `/review-fix-loop` · `/sdd` …）以及 `/profile` 预设选择器一键映射模型层级。
