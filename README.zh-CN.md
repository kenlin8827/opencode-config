...
终端界面提供对话式交互，支持 21 位专家智能体（`@java-dev`、`@security`、`@dba`、`@frontend-dev`、`@fast-coder` 等）、四种工作模式（`@lite`（默认 — 约 2k tok/步）/ `@code` / `@build` / `@plan`）、工作流斜杠命令（`/dev` · `/quick-dev` · `/plan-dev` · `/review-dev` · `/review-fix-loop` · `/sdd` …）以及 `/profile` 预设选择器一键映射模型层级。
