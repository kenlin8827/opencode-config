...
| `handoff.ts` (+ `handoff.md`) | `config` + `system.transform` | Registers `/handoff` slash command programmatically; injects handoff protocol (compact conversation into an OS-temp-dir document, reference artifacts by path, redact secrets, suggest agents) into system prompt (LLM-only, not visible in chat UI) |
