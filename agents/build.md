...
- **Autonomous multi-phase execution**: `/ultra-dev <objective> [--max-rounds=N] [--max-phases=N]` — decomposes into phases, each a `/deep-dev` cycle with context compaction, per-phase git-commit isolation, `--resume`. Safety guards: 10-round fuse, 3-consecutive-fuse hard stop, max-phases cap (default 6). Full protocol: `ultra-dev` skill.
